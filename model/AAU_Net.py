...
def parse_args():
    ...
    parser.add_argument("--active_type", type=str, default='Soft', help="the type of active function, opt: ReLU, Soft, Hard")
...
class UnrolledAutoEncoder(nn.Module):
    ...
    def __init__(self, opt):
        ...
        # Threshold Functions
        active_dict  ={'ReLU':ReLU, 'Soft':Soft, 'Hard':Hard}
        self.thrd1 = active_dict[opt.active_type](opt.CL[0])
        self.thrd2 = active_dict[opt.active_type](opt.CL[1])
        self.thrd3 = active_dict[opt.active_type](opt.CL[2])
        self.thrd4 = active_dict[opt.active_type](opt.CL[3])
    ...
    def forward(self, x, test=False):
        # Encoding
        gamma1 = self.thrd1(self.c1 * F.conv1d(x,      self.W1, stride=self.strd1, padding=self.pad1))
        gamma2 = self.thrd2(self.c2 * F.conv1d(gamma1, self.W2, stride=self.strd2, padding=self.pad2))
        gamma3 = self.thrd3(self.c3 * F.conv1d(gamma2, self.W3, stride=self.strd3, padding=self.pad3))
        gamma4 = self.thrd4(self.c4 * F.conv1d(gamma3, self.W4, stride=self.strd4, padding=self.pad4))

        for _ in range(self.T):
            # forward computation: gamma(i+1) = soft(gamma^(i+1)-c*DT*(D*gamma^(i+1)-gamma(i)))
            gamma1 = self.thrd1((gamma1 - self.c1 * F.conv1d(F.conv_transpose1d(gamma1, self.W1, stride=self.strd1, padding=self.pad1) - x,      self.W1, stride=self.strd1, padding=self.pad1)))
            gamma2 = self.thrd2((gamma2 - self.c2 * F.conv1d(F.conv_transpose1d(gamma2, self.W2, stride=self.strd2, padding=self.pad2) - gamma1, self.W2, stride=self.strd2, padding=self.pad2)))
            gamma3 = self.thrd3((gamma3 - self.c3 * F.conv1d(F.conv_transpose1d(gamma3, self.W3, stride=self.strd3, padding=self.pad3) - gamma2, self.W3, stride=self.strd3, padding=self.pad3)))
            gamma4 = self.thrd4((gamma4 - self.c4 * F.conv1d(F.conv_transpose1d(gamma4, self.W4, stride=self.strd4, padding=self.pad4) - gamma3, self.W4, stride=self.strd4, padding=self.pad4)))
